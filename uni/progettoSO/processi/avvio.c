...
Avvio menuIniziale(char *Nomeutente)
{
   ...
   wattron(finestraIniziale, COLOR_PAIR(DUE));
    for (i = 0; i < 3; i++)
        stampaRettangolo(finestraIniziale, 52, 22 + i * 7); // tra l'inizio di un rettangolo e l'inizio dell'altro ci sono 7 spazi
        ...
        if (input == KEY_MOUSE)
        {
            ...
            if (getmouse(&event) == OK)
            {
                ...
                { // click sinistro
                    ...
                    // mvwprintw(finestraIniziale, 0, 0, "Input ricevuto");
                    wrefresh(finestraIniziale);

                    if (event.x > INIZIO_RETTANGOLO_AVVIO && event.x < FINE_RETTANGOLO_AVVIO && event.y > 29 && event.y < 34)
                    { // USCITA
                        wclear(finestraIniziale);
                    ...
                    if (event.x > INIZIO_RETTANGOLO_AVVIO && event.x < FINE_RETTANGOLO_AVVIO && event.y > 15 && event.y < 20)
                    { // nuova partita
                        wclear(finestraIniziale);
                        wattron(finestraIniziale, COLOR_PAIR(DUE));
                        ...
                        while (true)
                        {
                            ...
                            else if (event.x > INIZIO_RETTANGOLO_AVVIO && event.x < FINE_RETTANGOLO_AVVIO && event.y > 22 && event.y < 27)
                            {
                                delwin(finestraIniziale);
                                info.difficolta = MEDIUM;
                                return info;
                            }
                            else if (event.x > INIZIO_RETTANGOLO_AVVIO && event.x < FINE_RETTANGOLO_AVVIO && event.y > 29 && event.y < 34)
                            {
                                delwin(finestraIniziale);
