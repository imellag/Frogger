    ...
    do
    {
        ...
        while (partitaInCorso)
        {
            ...
            else if (pacchetto.id >= PROIETTILE_NEMICO0_OUT && pacchetto.id <= PROIETTILE_NEMICO2_OUT)
            {
                ...
                funzPausa(finestraGioco, info.difficolta, camioncino, tronchino, macchinina, tempo.pid, ranocchio.pid);
            ...
            if (ranocchio.id == q || vite == 0 || buffer == false)
            {
                ...
                for (i = ZERO; i < NUMERO_MACCHINE + (info.difficolta * 3); i++)
                    kill(macchinina[i].pid, SIGKILL);

                for (i = ZERO; i < NUMERO_CAMION + (info.difficolta * 3); i++)
                    kill(camioncino[i].pid, SIGKILL);

                for (i = ZERO; i < NUMERO_TRONCHI; i++)
                    kill(tronchino[i].pid, SIGKILL);
